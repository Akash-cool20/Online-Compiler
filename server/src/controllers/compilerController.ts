...
export const saveCode = async (req: Request,res: Response)=>{
    ...
    try {
        ...
        return res.status(500).send({nessage: "Eroor saving code",error});
    }
}
...
export const loadCode =  async (req: Request,res: Response)=>{
    ...
    try {
        ...
        return res.status(201).send({fullCode: existingCode.fullCode});
    } catch (error) {
        return res.status(500).send({message: "Eroor in Loading code",error});
    }
}